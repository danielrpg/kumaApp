	    ...
	    while ($lin_tpa = mysql_fetch_array($res_tpa)) { // 1a
		 ...
		 if ($impo == $impo_e){
			if ($tip_tran == 1){
				...
				$con_cga  = "Select * From gral_cta_ingegre where gral_ingegre_tip = 1 and 
				           gral_ingegre_cta = $cta_ctble";
	            $res_cga = mysql_query($con_cga)or die('No pudo seleccionarse tabla gral_ingegre_tip')  ;	
	             while ($lin_ctble = mysql_fetch_array($res_cga )) {
			        $des_cta = $lin_ctble['gral_ingegre_desc'];
				}	
				...
				$con_cga  = "Select * From gral_cta_ingegre where gral_ingegre_tip = 2 and 
				           gral_ingegre_cta = $cta_ctble";
	            $res_cga = mysql_query($con_cga)or die('No pudo seleccionarse tabla gral_ingegre_tip')  ;	
	             while ($lin_ctble = mysql_fetch_array($res_cga )) {
			        $des_cta = $lin_ctble['gral_ingegre_desc'];
