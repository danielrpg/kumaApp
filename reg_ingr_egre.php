...
<html>
<head>
<title>Registro de Ingresos/Egresos por Operador</title>
...
$con_ope  = "Select * From gral_param_propios where GRAL_PAR_PRO_GRP = 700 
             and GRAL_PAR_PRO_COD <> 0 and GRAL_PAR_PRO_COD < 10 
             ORDER BY GRAL_PAR_PRO_COD LIMIT 0,10";
...
//$datos = $_SESSION['form_buffer'];
 ?>
  <table align="center">
    <tr>
     ...
     <tr>
	   ...
	   <tr>
		...
		<center>
	   <input type="submit" name="accion" value="Transacciones">
	...
	<?php	
	$con_cga  = "Select * From gral_cta_ingegre where gral_ingegre_tip = 1 
	             and gral_ingegre_cta = 110 ";
	$res_cga = mysql_query($con_cga)or die('No pudo seleccionarse tabla gral_ingegre_tip')  ;	
	while ($lin_cga = mysql_fetch_array($res_cga)) {
	      $_SESSION['tip_ing'] = $lin_cga['gral_ingegre_desc'];
	}
...
$res_ope = mysql_query($con_ope)or die('No pudo seleccionarse tabla')  ;	
	...
	<br>
	<?php
	 echo "Operador".encadenar(2).$_SESSION['nom_ope']; 
	?>
	<strong>
	<center>
	<?php
	 ...
	 <form name="form2" method="post" action="egre_retro_3.php" onSubmit="return ValidaCamposEgresos(this)">
	  ...
	  <table align="center">
	    ...
	    <td> <?php echo encadenar(2). $_SESSION['tip_ing']; ?>
...
<?php
$consulta  = "Select * From temp_ctable where temp_tipo = 1 order by temp_debe_2";
    ...
    while ($linea = mysql_fetch_array($resultado)) {
	 ...
	 <input type="submit" name="accion" value="Eliminar_Ingreso">
</form>	
<br>
...
<form name="form2" method="post" action="egre_retro_3.php" onSubmit="return ValidaCamposEgresos(this)">
  ...
  <tr>
	         <th align="left">Descripcion :</th>
			 <td><input type= type="text" name="desc_cre" size="30" maxlength="30" value="-" > </td>
			  <th align="left">Nro.Orden :</th>
			 <td><input type= type="text" name="nro_ord" size="10" maxlength="10" value="0" > </td> 
			 <th align="left" ><?php echo "Monto"; ?></th>
		     <td><input  type="text" name="cre_monto" value="0"> </td>
		 </tr>
  <tr>
              <th align="left">Fec. Servicio</th>
...
<table align="center" border="1">
	 <tr>
       	<td align="center"><strong>Nro.  </strong></td>
		<td align="center"><strong>Descripcion </strong></td>
		<td align="center"><strong>Nro.Orden </strong></td>
		<td align="center"><strong>Fec. Servicio </strong></td>
		<td align="center"><strong>Fec. Cobro </strong></td>
		<td align="center"><strong>Monto Bs. </strong></td>
		
   </tr>
   <?php
$consulta  = "Select * From temp_ctable where temp_tipo = 3 order by temp_debe_2";
    $resultado = mysql_query($consulta);
	...
	$tot_debe_1 = $tot_debe_1 +$linea['temp_debe_1'];
	      ...
	      <tr>
		       <td align="right"><?php echo number_format($linea['temp_debe_1'], 0, '.',',') ; ?></td>
	 	      <td align="left"><?php echo $linea['temp_des_cta']; ?></td>
			  <td align="right"><?php echo number_format($linea['temp_debe_2'],0, '.',''); ?></td>
		      <td align="right"><?php echo $linea['temp_fec1']; ?></td>
			  <td align="right"><?php echo $linea['temp_fec2']; ?></td>
			  <td align="right"><?php echo number_format($linea['temp_haber_1'], 2, '.',',') ; ?></td>
		      <td align="center"><INPUT NAME="cmone" TYPE=RADIO VALUE="<?php echo $linea['temp_debe_1']; ?>">	</td> 
	     </tr>
...
?>	
 ...
 <input type="submit" name="accion" value="Eliminar_Credito">
</form>
...
<?php
  ...
  $con_cga  = "Select * From gral_cta_ingegre where gral_ingegre_tip = 2  order by 3 ";
	$res_cga = mysql_query($con_cga)or die('No pudo seleccionarse tabla gral_ingegre_tip')  ;	
	 ?>
	 </strong>
	 <form name="form2" method="post" action="egre_retro_3.php" onSubmit="return ValidaCamposEgresos(this)">
...
<?php
$consulta  = "Select * From temp_ctable where temp_tipo = 2 order by temp_debe_2";
    $resultado = mysql_query($consulta);
    ...
    while ($linea = mysql_fetch_array($resultado)) {
	      $tip_gas = $linea['temp_nro_cta'];
		  $con_cga1  = "Select * From gral_cta_ingegre where gral_ingegre_tip = 2 and 
	 ...
	 <input type="submit" name="accion" value="Grab_Egresos">
	  <input type="submit" name="accion" value="Eliminar_Egresos">
    


<br><br>
	</table>	 
	 <input type="submit" name="accion" value="Grab_Planilla">  
...
if ($_SESSION['detalle'] == 3){  //1a
    $con_trc = "SELECT count(*) FROM temp_ctable where temp_tipo = 1";
   $res_trc = mysql_query($con_trc)or die('No pudo seleccionarse tabla temp_tipo 1');
...
$consulta = "insert into temp_ctable (   temp_tipo, 
                                          temp_tip_tra,
	                                      temp_nro_cta, 
                                          temp_des_cta,
						 	              temp_debe_1,
...
?>
 ...
 if ($_SESSION['eliminar'] == 1){
  ...
  <?php
	 $consulta  = "Delete From temp_ctable where temp_debe_1 = $cmone and  temp_tipo = 1";
     $resultado = mysql_query($consulta);
	...
	$nro = 0;
	 ...
	 $nro1 = $linea['temp_debe_1'];
	  $con_modi  = "update temp_ctable set temp_debe_1 = $nro
	                                      where temp_debe_1 = $nro1 and  temp_tipo = 1";
     $res_modi = mysql_query($con_modi);
...
if ($_SESSION['eliminar'] == 2){
   // echo $_SESSION['entra'];
    if(isset($_POST['cmone'])){ //2a
	 ...
	 <form name="form2" method="post" action="con_retro_1.php" onSubmit="">
  <?php
	 $consulta  = "Delete From temp_ctable where temp_debe_1 = $cmone and  temp_tipo = 2";
     $resultado = mysql_query($consulta);
	...
	$nro = 0;
	 ...
	 $nro1 = $linea['temp_debe_1'];
	  $con_modi  = "update temp_ctable set temp_debe_1 = $nro
	                                      where temp_debe_1 = $nro1 and  temp_tipo = 2";
     $res_modi = mysql_query($con_modi);
...
if ($_SESSION['eliminar'] == 3){
   // echo $_SESSION['entra'];
    if(isset($_POST['cmone'])){ //2a
	 ...
	 <form name="form2" method="post" action="con_retro_1.php" onSubmit="">
  <?php
	 $consulta  = "Delete From temp_ctable where temp_debe_1 = $cmone and  temp_tipo = 3";
     $resultado = mysql_query($consulta);
	...
	$nro = 0;
	 ...
	 $nro1 = $linea['temp_debe_1'];
	  $con_modi  = "update temp_ctable set temp_debe_1 = $nro
	                                      where temp_debe_1 = $nro1 and  temp_tipo = 3";
     $res_modi = mysql_query($con_modi);
...
//Grabar Credito en temp
if ($_SESSION['detalle'] == 4){  //1a
    $con_trc = "SELECT count(*) FROM temp_ctable where temp_tipo = 3";
...
if ($_POST['desc_cre'] <> ""){  
	$desc_cre = $_POST['desc_cre'];
	$desc_cre = strtoupper ($desc_cre);
	$_SESSION['desc_cre'] = $desc_cre;
	}

 if ($_POST['cre_monto'] > 0){  
    if ($_SESSION['egre_bs_sus'] == 2){
	   $_SESSION['monto_eq'] = $_POST['egr_monto'];
	     $monto_eq = $_SESSION['monto_eq'];
         $monto_t = (($_POST['egr_monto'] * $_SESSION['tc_ctb']));
      }else{
	            $cre_monto = ($_POST['cre_monto']);
	  }			
				$_SESSION['cre_monto'] =  $cre_monto;
	}
$nro_ord = $_POST['nro_ord'];
$_SESSION['nro_ord'] =  $nro_ord;
echo $nro."nro aqui".$_SESSION['cod_ope']."ope";
$cod_ope = $_SESSION['cod_ope'];
$fec_serv = $_POST['fec_des'];
$fec_cob = $_POST['fec_uno'];
$fec1 = cambiaf_a_mysql_2($fec_serv);
$fec2 = cambiaf_a_mysql_2($fec_cob);
...
<?php
$consulta  = "Select * From temp_ctable order by temp_debe_1";
    $resultado = mysql_query($consulta);
...
// Graba Egresos
if ($_SESSION['detalle'] == 5){ 

//echo "graba Aqui";
  $con_trc = "SELECT count(*) FROM temp_ctable where temp_tipo = 2";
   $res_trc = mysql_query($con_trc)or die('No pudo seleccionarse tabla cajero');
   while ($lin_trc = mysql_fetch_array($res_trc)) {
         $nro =  $lin_trc['count(*)'];
      }
