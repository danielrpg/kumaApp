...
<html>
<head>
<title>Respaldo Cronograma Diario</title>
...
if ($_SESSION['dia'] == 1) {
   ...
   $_SESSION['fec_nuer'] = cambiaf_a_normal($fecha);
}   
if ($_SESSION['dia'] == 4) {
   $hoy = date("Y-m-d");
   $fecha = $hoy;
...
if ($_SESSION['dia'] == 2) {
 $ndias = 1;
   $fec = cambiaf_a_normal($_SESSION['fechar']);
...
//for ($x=1; $x < 14; $x = $x + 1 ) {
    //for ($y=1; $y < 5; $y = $y + 1 ) {
	//$nro_ope =$nro_ope + 1; 
$con_sol  = "Select * From ord_conobck where ord_bck_fecha = '$fecha'"; 
$res_sol = mysql_query($con_sol)or die('No pudo seleccionarse ord_cronograma');
   while ($lin_sol = mysql_fetch_array($res_sol)) {
        $nro_ope = 0;
 
        $nro_ope = $lin_sol['ord_bck_ope'];
		//echo "Aqui".$nro_ope;
	  if ($nro_ope == 1) {
		  $hra_6_det[$nro_ope] = $lin_sol['ord_bck_6_det'];
		  $hra_7_det[$nro_ope] = $lin_sol['ord_bck_7_det'];
		  $hra_8_det[$nro_ope] = $lin_sol['ord_bck_8_det'];
		  $hra_9_det[$nro_ope] = $lin_sol['ord_bck_9_det'];	
		  $hra_10_det[$nro_ope] = $lin_sol['ord_bck_10_det'];
		  $hra_11_det[$nro_ope] = $lin_sol['ord_bck_11_det'];
		  $hra_12_det[$nro_ope] = $lin_sol['ord_bck_12_det'];
		  $hra_13_det[$nro_ope] = $lin_sol['ord_bck_13_det']; 
		  $hra_14_det[$nro_ope] = $lin_sol['ord_bck_14_det'];
		  $hra_15_det[$nro_ope] = $lin_sol['ord_bck_15_det'];
		  $hra_16_det[$nro_ope] = $lin_sol['ord_bck_16_det']; 
		  $hra_17_det[$nro_ope] = $lin_sol['ord_bck_17_det']; 
		  $hra_18_det[$nro_ope] = $lin_sol['ord_bck_18_det'];
		  $hra_19_det[$nro_ope] = $lin_sol['ord_bck_ot_det']; 
		 //  echo $hra_ot_det[$nro_ope]."---". $lin_sol['ord_bck_ot_det'];
			}
		 if ($nro_ope == 2) {	
		    $hra_6_det[$nro_ope] = $lin_sol['ord_bck_6_det'];
		  $hra_7_det[$nro_ope] = $lin_sol['ord_bck_7_det'];
	...
	<form name="form2" method="post" action="solic_retro_bck.php" onSubmit="return ValidarRangoFechas(this)" >
	...
	<font size="+2">
	 ...
	 //echo $dia." ".$mes." ".$anio;
	 echo "Fecha" .encadenar(2). $_SESSION['fec_nuer'].encadenar(2).$dia_sem; 
	 //echo horac30();
	 ?>
	 </center>
	 <?php
...
<div id="FooterTable">
<BR><B><FONT SIZE=+2><MARQUEE>Respaldo *** Cronograma Diario *** </MARQUEE></FONT></B>
</div>
   <?php
